...
describe('Decoy Server', () => {
  HTTP_ADAPTERS.forEach((transport) => {
    ...
    describe('ResourceRouteBuilder - selective', () => {
      let server: DecoyServer<any>
      let client: TestHttpClient
      ...
      beforeAll(async () => {
        server = await makeDecoyServer({
          ...
          endpoints: {
            'api/species': new ResourceRouteBuilder(
              'api/species',
              new ArrayCollection(ANIMAL_SPECIES_RECORDS),
              'id'
            )
              .route((r) => r.getAll())
        ...
        client = await makeTestClient(server)
        console.dir(server.routes, { depth: null })
      })
      ...
      ANIMAL_SPECIES_RECORDS.forEach((record) => {
        it(`should respond with record with ID=${record.id}("${record.name}") at GET /api/species/${record.id}`, async () => {
          // When
          const response = await client!.get(`/api/species/${record.id}`)
          ...
          expect(server.requestLog.statusCodes()).toEqual([200])
          console.log(server.requestLog)
          expect(server.requestLog.byRouteId('api/species-GET(id)')).toHaveLength(1)
        })
      })

      it('should respond with 404 at GET /species/10', async () => {
        // When
        const response = await client!.get(`api/species/10`)
      ...
      it('should accept a POST to create a new record at /species', async () => {
        ...
        // When
        const response = await client!.post(`/api/species`, payload)
        ...
        expect(server.requestLog.statusCodes()).toEqual([201])
        expect(server.requestLog.byRouteId('api/species-POST')).toHaveLength(1)
      })

      it('should respond with 404 for PUT /species/4', async () => {
        // Given
        const payload = {
          name: 'Three-legged Ribbet-Ribbet Sound Maker',
          class: 'Amphibia',
        ...
        // When
        const response = await client!.put(`/api/species/4`, payload)

        // Then
        expect(response.status).toBe(404)

        expect(server.requestLog.statusCodes()).toEqual([])
        expect(server.requestLog.byRouteId('api/species-PUT(id)')).toHaveLength(0)
      })

      it('should respond with 404 for DELETE api/species/2', async () => {
        // When
        const response = await client!.delete('species/2')
        ...
        expect(server.requestLog.statusCodes()).toEqual([])
        expect(server.requestLog.byRouteId('api/species-DELETE(id)')).toHaveLength(0)
      })
    })
  })
})
